...
class APKCommandTool:
    def __init__(self, root):
        ...
        # AAPT工具路径
        #self.aapt_path = "C:/Users/BCTC/AppData/Local/Programs/Python/Python311/Lib/site-packages/drozer/lib/aapt.exe"  绝对路径
        # 获取当前脚本所在目录
    ...
    # 第一行：APK文件选择
        ...
        tk.Entry(self.root, textvariable=self.target_server, width=25, font=self.font).grid(row=row, column=1, padx=5, pady=5, sticky="w")
        
        Label(self.root, text="进程名:", font=self.font).grid(row=row, column=1, padx=5, pady=5, sticky="e")
        tk.Entry(self.root, textvariable=self.process_name, width=20, font=self.font).grid(row=row, column=2, padx=5, pady=5, sticky="w")
        ...
        # 新增：IPv6测试按钮，放在通讯安全检测旁边
        Button(self.other_frame, text="IPv6测试", command=self.check_ipv6_support, font=self.font, width=10).pack(side="left", padx=2)
